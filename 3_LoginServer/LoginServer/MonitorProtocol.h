...
#pragma pack(push, 1)
struct st_PACKET_SS_MONITOR_LOGIN {
	WORD Type = en_PACKET_SS_MONITOR_LOGIN;
	int ServerNo = -1;
};

struct st_PACKET_SS_MONITOR_DATA_UPDATE {
...
struct st_PACKET_CS_MONITOR_TOOL_REQ_LOGIN {
	WORD Type = en_PACKET_CS_MONITOR_TOOL_REQ_LOGIN;
	char LoginSessionKey[32];
};

struct st_PACKET_CS_MONITOR_TOOL_RES_LOGIN {
	WORD Type = en_PACKET_CS_MONITOR_TOOL_RES_LOGIN;
	BYTE Status = 0;
};

struct st_PACKET_CS_MONITOR_TOOL_DATA_UPDATE {
	WORD Type = 0;
	BYTE ServerNo = -1;
	BYTE DataType = 0;
	int DataValue = -1;
